...
from mil_models.model_utils import *
...
class TMIL(nn.Module):
    def __init__(self, omic_input_dim=None, fusion=None, size_arg="small", dropout=0.25,
                 n_classes=4, df_comp=None, dim_per_path_1=16, dim_per_path_2=64,
                 device="cpu", wsi_input_dim=768):
        r"""
        Attention MIL Implementation
        ...
        self.device = device
        self.fusion = fusion
        self.size_dict_path = {"small": [wsi_input_dim, 256, 256], "big": [wsi_input_dim, 512, 384]}
        ...
        self.num_pathways = self.df_comp.shape[1] if self.df_comp is not None else 0
    ...
    def forward(self, **kwargs):
        ...
        if x_path.dim() != 3 or x_path.shape[0] != 1:
            raise ValueError(f"TransMIL expects [1,patches,dim], got {tuple(x_path.shape)}")
        x_path = self.fc(x_path)
        h_path = self.nystrom(x_path)
        h_path = h_path.squeeze(0).mean(dim=0)
