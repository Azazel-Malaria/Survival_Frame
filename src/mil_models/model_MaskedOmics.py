...
class MaskedOmics(nn.Module):
    def __init__(
        ...
        self.register_buffer('mask_1', torch.repeat_interleave(M_raw, self.dim_per_path_1, dim=1))
        ...
        mask_2 = np.zeros([self.dim_per_path_1*self.num_pathways, self.dim_per_path_2*self.num_pathways])
        for (row, col) in zip(range(0, self.dim_per_path_1*self.num_pathways, self.dim_per_path_1), range(0, self.dim_per_path_2*self.num_pathways, self.dim_per_path_2)):
            mask_2[row:row+self.dim_per_path_1, col:col+self.dim_per_path_2] = 1
        self.register_buffer('mask_2', torch.Tensor(mask_2))
